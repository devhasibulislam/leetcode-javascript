 ...
 * Description: CombinationIterator(string characters, int combinationLength) Initializes the object with a string characters of sorted distinct lowercase English letters and a number combinationLength as arguments.
...
var CombinationIterator = function (characters, combinationLength) {
  ...
  const buildAllCombination = (curString, masterString) => {
    ...
    for (let g = 0; g < masterString.length; g++) {
      buildAllCombination(
        curString + masterString[g],
        masterString.slice(g + 1)
      );
    }
  };
  //lets sort it first
